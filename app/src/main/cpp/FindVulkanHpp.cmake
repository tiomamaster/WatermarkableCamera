...
#
# VulkanHpp_FOUND VulkanHpp_INCLUDE_DIRS
#
...
function(extract_vulkan_version VULKAN_CORE_H_PATH OUTPUT_VERSION_TAG)
  ...
  file(STRINGS ${VULKAN_CORE_H_PATH} VULKAN_VERSION_MAJOR_LINE
       REGEX "^#define VK_VERSION_MAJOR")
  file(STRINGS ${VULKAN_CORE_H_PATH} VULKAN_VERSION_MINOR_LINE
       REGEX "^#define VK_VERSION_MINOR")
  file(STRINGS ${VULKAN_CORE_H_PATH} VULKAN_HEADER_VERSION_LINE
       REGEX "^#define VK_HEADER_VERSION")

  set(VERSION_TAG "v1.3.275") # Default fallback

  if(VULKAN_VERSION_MAJOR_LINE
     AND VULKAN_VERSION_MINOR_LINE
     AND VULKAN_HEADER_VERSION_LINE)
    string(REGEX REPLACE "^#define VK_VERSION_MAJOR[ \t]+([0-9]+).*$" "\\1"
                         VULKAN_VERSION_MAJOR "${VULKAN_VERSION_MAJOR_LINE}")
    string(REGEX REPLACE "^#define VK_VERSION_MINOR[ \t]+([0-9]+).*$" "\\1"
                         VULKAN_VERSION_MINOR "${VULKAN_VERSION_MINOR_LINE}")
    string(REGEX REPLACE "^#define VK_HEADER_VERSION[ \t]+([0-9]+).*$" "\\1"
                         VULKAN_HEADER_VERSION "${VULKAN_HEADER_VERSION_LINE}")

    # Construct the version tag
    set(VERSION_TAG
        "v${VULKAN_VERSION_MAJOR}.${VULKAN_VERSION_MINOR}.${VULKAN_HEADER_VERSION}"
    )
  else()
    ...
    if(VULKAN_HEADER_VERSION_COMPLETE_LINE AND VULKAN_HEADER_VERSION_LINE)
      ...
      string(REGEX
             REPLACE "^#define VK_HEADER_VERSION[ \t]+([0-9]+).*$" "\\1"
                     VULKAN_HEADER_VERSION "${VULKAN_HEADER_VERSION_LINE}")

      # Check if the complete version line contains the major and minor versions
      if(VULKAN_HEADER_VERSION_COMPLETE_LINE
         MATCHES
         "VK_MAKE_API_VERSION\\(.*,[ \t]*([0-9]+),[ \t]*([0-9]+),[ \t]*VK_HEADER_VERSION\\)"
      )
...
if(DEFINED ANDROID_NDK)
  ...
  if(VULKAN_CORE_H)
    ...
    message(
      STATUS
        "Could not find vulkan_core.h in NDK, using default version: ${VULKAN_VERSION_TAG}"
    )
  endif()
...
elseif(DEFINED ENV{VULKAN_SDK})
  ...
  find_file(
    VULKAN_CORE_H vulkan_core.h
    PATHS $ENV{VULKAN_SDK}/include/vulkan
    NO_DEFAULT_PATH)
  ...
  if(VULKAN_CORE_H)
    ...
    message(
      STATUS
        "Could not find vulkan_core.h in Vulkan SDK, using default version: ${VULKAN_VERSION_TAG}"
    )
  endif()
...
elseif(DEFINED Vulkan_INCLUDE_DIR)
  ...
  find_file(
    VULKAN_CORE_H vulkan_core.h
    PATHS ${Vulkan_INCLUDE_DIR}/vulkan
    NO_DEFAULT_PATH)
  ...
  if(VULKAN_CORE_H)
    ...
    message(
      STATUS
        "Detected Vulkan version from include directory: ${VULKAN_VERSION_TAG}")
  else()
    message(
      STATUS
        "Could not find vulkan_core.h in Vulkan include directory, using default version: ${VULKAN_VERSION_TAG}"
    )
  endif()
else()
  # Try to find vulkan_core.h in system paths
  find_file(VULKAN_CORE_H vulkan_core.h PATHS /usr/include/vulkan
  ...
  if(VULKAN_CORE_H)
    ...
    message(
      STATUS
        "Could not find vulkan_core.h in system paths, using default version: ${VULKAN_VERSION_TAG}"
    )
  endif()
endif()
...
if(NOT VulkanHpp_INCLUDE_DIR OR NOT VulkanHpp_CPPM_DIR)
  ...
  message(
    STATUS
      "Vulkan-Hpp not found, fetching from GitHub with version ${VULKAN_VERSION_TAG}..."
  )
  ...
  if(EXISTS "${VulkanHpp_SOURCE_DIR}/vulkan/vulkan.cppm")
    ...
    file(
      WRITE "${VulkanHpp_CPPM_DIR}/vulkan/vulkan.cppm"
...
# If the Vulkan Profiles include directory wasn't found, use FetchContent to
# download
if(NOT VulkanProfiles_INCLUDE_DIR)
  ...
  message(
    STATUS "Vulkan-Profiles not found, fetching from GitHub main branch...")
  FetchContent_Declare(
    VulkanProfiles
  ...
  # Populate the content
  FetchContent_GetProperties(VulkanProfiles
  ...
  set(VulkanProfiles_INCLUDE_DIR
      ${CMAKE_CURRENT_BINARY_DIR}/VulkanProfiles/include)
  ...
  if(NOT EXISTS "${VulkanProfiles_INCLUDE_DIR}/vulkan/vulkan_profiles.hpp")
    file(
      WRITE "${VulkanProfiles_INCLUDE_DIR}/vulkan/vulkan_profiles.hpp"
...
find_package_handle_standard_args(
  ...
  FAIL_MESSAGE
    "Could NOT find VulkanHpp. Install it or set VulkanHpp_INCLUDE_DIR to the directory containing vulkan/vulkan.hpp"
)
...
if(VulkanHpp_FOUND)
  ...
  if(NOT DEFINED VulkanHpp_CPPM_DIR)
    ...
    if(EXISTS "${VulkanHpp_INCLUDE_DIR}/vulkan/vulkan.cppm")
      ...
      message(
        STATUS
          "Found vulkan.cppm in VulkanHpp_INCLUDE_DIR: ${VulkanHpp_CPPM_DIR}")
    elseif(DEFINED VulkanHpp_SOURCE_DIR
           AND EXISTS "${VulkanHpp_SOURCE_DIR}/vulkan/vulkan.cppm")
      set(VulkanHpp_CPPM_DIR ${VulkanHpp_SOURCE_DIR})
      message(
        STATUS
          "Found vulkan.cppm in VulkanHpp_SOURCE_DIR: ${VulkanHpp_CPPM_DIR}")
    elseif(DEFINED vulkanhpp_SOURCE_DIR
           AND EXISTS "${vulkanhpp_SOURCE_DIR}/vulkan/vulkan.cppm")
      set(VulkanHpp_CPPM_DIR ${vulkanhpp_SOURCE_DIR})
      message(
...
")
  ...
  if(VulkanProfiles_INCLUDE_DIR
     AND EXISTS "${VulkanProfiles_INCLUDE_DIR}/vulkan/vulkan_profiles.hpp")
    list(APPEND VulkanHpp_INCLUDE_DIRS ${VulkanProfiles_INCLUDE_DIR})
    message(
      STATUS
        "Added Vulkan Profiles include directory: ${VulkanProfiles_INCLUDE_DIR}"
    )
  endif()
  ...
  if(NOT TARGET VulkanHpp::VulkanHpp)
    ...
    set_target_properties(
      VulkanHpp::VulkanHpp PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                      "${VulkanHpp_INCLUDE_DIRS}")
  endif()
elseif(DEFINED VulkanHpp_SOURCE_DIR OR DEFINED vulkanhpp_SOURCE_DIR)
  # If find_package_handle_standard_args failed but we have a VulkanHpp source
  # directory from FetchContent Create an imported target
  if(NOT TARGET VulkanHpp::VulkanHpp)
    add_library(VulkanHpp::VulkanHpp INTERFACE IMPORTED)
    ...
    message(
      STATUS
        "Using fallback VulkanHpp source directory: ${_vulkanhpp_source_dir}")

    set_target_properties(
      VulkanHpp::VulkanHpp PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                      "${_vulkanhpp_source_dir}")
  endif()
  ...
  if(VulkanProfiles_INCLUDE_DIR
    ...
    message(
      STATUS
        "Added Vulkan Profiles include directory to fallback: ${VulkanProfiles_INCLUDE_DIR}"
    )
  endif()
  ...
  if(NOT DEFINED VulkanHpp_CPPM_DIR)
    # Check if vulkan.cppm exists in the downloaded repository
    if(DEFINED VulkanHpp_INCLUDE_DIR
       AND EXISTS "${VulkanHpp_INCLUDE_DIR}/vulkan/vulkan.cppm")
      set(VulkanHpp_CPPM_DIR ${VulkanHpp_INCLUDE_DIR})
      message(
        STATUS
          "Found vulkan.cppm in VulkanHpp_INCLUDE_DIR: ${VulkanHpp_CPPM_DIR}")
    elseif(DEFINED _vulkanhpp_source_dir
           AND EXISTS "${_vulkanhpp_source_dir}/vulkan/vulkan.cppm")
      set(VulkanHpp_CPPM_DIR ${_vulkanhpp_source_dir})
      message(
        STATUS
          "Found vulkan.cppm in _vulkanhpp_source_dir: ${VulkanHpp_CPPM_DIR}")
    elseif(DEFINED VulkanHpp_SOURCE_DIR
           AND EXISTS "${VulkanHpp_SOURCE_DIR}/vulkan/vulkan.cppm")
      set(VulkanHpp_CPPM_DIR ${VulkanHpp_SOURCE_DIR})
      message(
