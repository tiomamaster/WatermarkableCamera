...
NDKCamera::~NDKCamera() {
    ...
    if (cameraMgr_) {
        CALL_MGR(
            unregisterAvailabilityCallback(cameraMgr_, GetManagerListener()));
    ...
    for (int i = 0; i < cameraIds->numCameras; ++i) {
        ...
        for (int tagIdx = 0; tagIdx < count; ++tagIdx) {
            if (ACAMERA_LENS_FACING == tags[tagIdx]) {
                ...
                CALL_METADATA(
                    getConstEntry(metadataObj, tags[tagIdx], &lensInfo));
                CameraId cam(id);
                cam.facing_ =
...
bool NDKCamera::TakePhoto(void) {
    ...
    CALL_SESSION(
        capture(captureSession_, GetCaptureCallback(), 1,
...
void NDKCamera::UpdateCameraRequestParameter(int32_t code, int64_t val) {
    ...
    switch (code) {
        case ACAMERA_SENSOR_EXPOSURE_TIME:
            if (exposureRange_.Supported()) {
                exposureTime_ = val;
                CALL_REQUEST(setEntry_i64(request, ACAMERA_SENSOR_EXPOSURE_TIME,
                                          1, &exposureTime_));
            }
            break;
        ...
        case ACAMERA_SENSOR_SENSITIVITY:
            if (sensitivityRange_.Supported()) {
                sensitivity_ = val;
                CALL_REQUEST(setEntry_i32(request, ACAMERA_SENSOR_SENSITIVITY,
                                          1, &sensitivity_));
            }
            break;
        default:
    ...
    CALL_SESSION(setRepeatingRequest(
        captureSession_, nullptr, 1, &request,
